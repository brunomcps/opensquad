...
function emptyBucket(bucketStart: string): Bucket {
  return {
    bucketStart,
    clicks: { description: 0, pinnedComment: 0, commentReply: 0, video: 0, other: 0, total: 0, unknown: 0 },
    sales: { description: 0, pinnedComment: 0, commentReply: 0, video: 0, unattributed: 0, ambiguous: 0, additional: 0, total: 0 },
    financialDataIncomplete: 0,
    netAfterFees: 0,
  };
}

function clickPosition(bucket: Bucket, position: string | null, count: number): void {
  if (position === 'description') bucket.clicks.description += count;
...
function salePosition(bucket: Bucket, position: string | null, count: number): void {
  if (position === 'description') bucket.sales.description += count;
  ...
  try {
    ...
    for (const row of seriesResult.data || []) {
      ...
      if (row.event_type === 'click') {
        ...
        clickPosition(bucket, row.cta_position, count);
        ...
        if (row.attribution === 'direct_primary') {
          salePosition(bucket, row.cta_position, count);
