...
import { LinkedInIcon } from './icons/PlatformIcons';
...
export function LinkedInDetail() {
  const selectedId = useContentStore((s) => s.selectedLinkedinId);
  const posts = useContentStore((s) => s.linkedinPosts);
  ...
  return (
    <div style={getOverlayStyle(expanded)} onClick={(e) => { if (e.target === e.currentTarget) setSelectedLinkedinId(null); }}>
      <div style={getPanelStyle(expanded)}>
        <div style={topBar}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-muted)', fontFamily: 'var(--font)' }}>Detalhes — LinkedIn</span>
          ...
          <button style={btn} onClick={() => { setSelectedLinkedinId(null); setExpanded(false); }}>Fechar</button>
        </div>
