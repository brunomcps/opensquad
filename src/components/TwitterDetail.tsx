...
import { XIcon } from './icons/PlatformIcons';
...
export function TwitterDetail() {
  const selectedId = useContentStore((s) => s.selectedTwitterId);
  const posts = useContentStore((s) => s.twitterPosts);
  ...
  return (
    <div style={getOverlayStyle(expanded)} onClick={(e) => { if (e.target === e.currentTarget) setSelectedTwitterId(null); }}>
      <div style={getPanelStyle(expanded)}>
        <div style={topBar}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-muted)', fontFamily: 'var(--font)' }}>Detalhes — X</span>
          ...
          <button style={btn} onClick={() => { setSelectedTwitterId(null); setExpanded(false); }}>Fechar</button>
        </div>
        ...
        <div style={body}>
          ...
          <div style={statsGrid}>
            <div style={statCard}>
              <div style={statVal}>{formatNumber(post.viewCount)}</div>
            ...
            <div style={statCard}>
              <div style={statVal}>{formatNumber(post.retweetCount)}</div>
              <div style={statLbl}>Reposts</div>
            </div>
            <div style={statCard}>
          ...
          <div>
            ...
            <div style={infoRow}>
              ...
              <span style={{ fontWeight: 600 }}>{formatDate(post.createdAt)}</span>
            </div>
          </div>
