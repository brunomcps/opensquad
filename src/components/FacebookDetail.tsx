...
import { FacebookIcon } from './icons/PlatformIcons';
...
const POST_TYPES: Record<string, string> = {
  link: 'Link', status: 'Status', photo: 'Foto', video: 'Video',
};

export function FacebookDetail() {
  const selectedId = useContentStore((s) => s.selectedFacebookId);
  const posts = useContentStore((s) => s.facebookPosts);
  ...
  return (
    <div style={getOverlayStyle(expanded)} onClick={(e) => { if (e.target === e.currentTarget) setSelectedFacebookId(null); }}>
      <div style={getPanelStyle(expanded)}>
        <div style={topBar}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-muted)', fontFamily: 'var(--font)' }}>Detalhes — Facebook</span>
          ...
          <button style={btn} onClick={() => { setSelectedFacebookId(null); setExpanded(false); }}>Fechar</button>
        </div>

        {post.fullPicture && (
          <div style={{ display: 'flex', justifyContent: 'center', background: 'var(--bg-primary)' }}>
            <img src={post.fullPicture} alt="" style={thumbImg} />
          </div>
        )}
        ...
        <div style={body}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{POST_TYPES[post.type] || post.type}</span>
            </div>
          </div>
          ...
          <div style={statsGrid}>
            ...
            <div style={statCard}>
              <div style={statVal}>{formatNumber(post.commentCount)}</div>
              <div style={statLbl}>Comentarios</div>
            </div>
            <div style={statCard}>
              <div style={statVal}>{formatNumber(post.shareCount)}</div>
              <div style={statLbl}>Compartilhamentos</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={infoRow}>
              ...
              <span style={{ fontWeight: 600 }}>{formatDate(post.createdTime)}</span>
            </div>
            <div style={infoRow}>
              <span style={{ color: 'var(--text-muted)' }}>Tipo</span>
              <span style={{ fontWeight: 600 }}>{POST_TYPES[post.type] || post.type}</span>
            </div>
          </div>

          <div>
            <div style={sectionTitle}>Texto</div>
            <div style={captionBox}>{post.message || 'Sem texto'}</div>
          </div>
