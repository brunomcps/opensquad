...
test('campanhas exigem membro e mutações exigem admin sem expor autor no DTO', () => {
  const source = fs.readFileSync(
    path.resolve(directory, '../../../supabase/functions/ci-campaigns/index.ts'),
    'utf8',
  );
  assert.match(source, /authorizeMember\(request, client, 'viewer'\)/);
  assert.match(source, /authorizeMember\(request, client, 'admin'\)/);
  assert.match(source, /created_by:\s*member\.userId/);
  assert.match(source, /created_by:\s*_createdBy/);
  assert.doesNotMatch(source, /buyer_key|buyer.*email|document|phone/i);
});

test('redirect prioriza destino e não persiste IP ou URL completa de referência', () => {
  const source = fs.readFileSync(
